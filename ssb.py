...
def unknown(bot, update):
    bot.sendMessage(
        ...
        text="Sorry, but Sasuma didn't understand that command 👵🏻"
    )
...
def unknown_text(bot, update):
    bot.sendMessage(
        chat_id=update.message.chat_id,
        text=(
            "Sasuma's very efficient, "
            "she doesn't have time for small talk."
            "Just give her a command, and she'll do it 👵🏻."
            "Have a look [here](https://github.com/iamgroot42/sasuma/)"
...
def long_command(bot, update, job_queue):
    ...
    job = job_queue.run_repeating(
        callback_screencheck,
        FREQUENCY,
        context=context_obj
    )
    running_jobs[screen_name] = job
    update.message.reply_text(
        "Job started 👍🏻! Output will written to %s" % (unique_filename)
    )
...
def callback_screencheck(bot, job):
    ...
    if not screen_running(job.context['screename']):
        if len(open(job.context['filename'], 'rb').read()) != 0:
            bot.send_message(
                chat_id=job.context['userid'],
                text="Sasuma's done with the job 👩🏻‍💻! Here's the log:"
            )
            bot.send_document(
                chat_id=job.context['userid'],
                document=open(job.context['filename'], 'rb')
            )
        else:
            bot.send_message(
                chat_id=job.context['userid'],
                text=(
                    "Sasuma's done with the job 👩🏻‍💻!"
                    " There was no output, though"
                )
            )
